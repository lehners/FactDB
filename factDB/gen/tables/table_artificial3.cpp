...
/*---------------------------------------------------------------------------*/
void artificial3::loadRelation(const std::string& path, char separator) {
   assert(!loaded);
...
/*---------------------------------------------------------------------------*/
void artificial3::unloadRelation() {
   data.clear();
   loaded = false;
}
/*---------------------------------------------------------------------------*/
RuntimeValue artificial3::get(size_t idx, const IU& iu) const {
   if ("a" == iu.column) {
...
/*---------------------------------------------------------------------------*/
void artificial3::genTblFiles(const std::string& tblPath, char separator) const {
   genTblFileImpl<artificial3, Container>(tblPath, separator);
}
/*---------------------------------------------------------------------------*/
schemac::Table artificial3::genSchema() {
   std::vector<factDB::schemac::Column> cols = {
      {"a", schemac::Type::Integer()},
      {"b", schemac::Type::Integer()},
      {"id", schemac::Type::Integer()}};
   return {411, "artificial3", std::move(cols)};
}
/*---------------------------------------------------------------------------*/
factDB::BaseTable* create_artificial3() { return new artificial3(); }
/*---------------------------------------------------------------------------*/
void destroy_artificial3(factDB::BaseTable* ptr) { delete ptr; }
/*---------------------------------------------------------------------------*/
} // namespace factDB::tables
/*---------------------------------------------------------------------------*/
