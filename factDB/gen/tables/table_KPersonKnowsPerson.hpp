...
/*---------------------------------------------------------------------------*/
struct KPersonKnowsPerson final : public BaseTable {
   enum Columns { creationDate,
                  person1,
                  person2 };

   struct Container {
      using SingleNodeTreeType = SingleNodeTree<Timestamp, UInt64, UInt64>;
      SingleNodeTreeType::ElementTuple tuple;

      decltype(auto) creationDate(this auto&& self) { return std::get<0>(self.tuple); }
      decltype(auto) person1(this auto&& self) { return std::get<1>(self.tuple); }
      decltype(auto) person2(this auto&& self) { return std::get<2>(self.tuple); }
   ...
   ~KPersonKnowsPerson() override = default;
   std::string_view name() const override { return "KPersonKnowsPerson"; };
...
factDB::BaseTable* create_KPersonKnowsPerson();
void destroy_KPersonKnowsPerson(factDB::BaseTable* ptr);
/*---------------------------------------------------------------------------*/
} // namespace factDB::tables
/*---------------------------------------------------------------------------*/
