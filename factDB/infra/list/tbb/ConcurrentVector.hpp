...
// #include "tbb/detail/_containers_helpers.h"
// #include "tbb/detail/_namespace_injection.h"
// #include "tbb/detail/_utils.h"
#include "factDB/infra/list/tbb/SegmentTable.hpp"
...
namespace factDB::infra::list::tbb {
...
class vector_iterator {
   ...
   reference operator*() const {
      ...
      if (item == nullptr) {
         ...
         assert(item == &my_vector->internal_subscript(my_index) && "corrupt cache");
      }
      return *item;
   }
   ...
   vector_iterator& operator--() {
      assert(my_index > 0 && "operator--() applied to iterator already at beginning of concurrent_vector");
      --my_index;
      if (my_item != nullptr) {
         if (vector_type::is_first_element_in_segment(my_index)) {
            // If the iterator crosses a segment boundary, the pointer become invalid
   ...
   bool isLast() const { return my_vector == nullptr || *this == my_vector->end(); }
   void setEnd() { my_vector = nullptr; }
   [[nodiscard]] size_t index() const { return my_index; }
...
static constexpr std::size_t embedded_table_num_segments = 0;

template <typename T, typename Allocator = oneapi::tbb::cache_aligned_allocator<T>>
class concurrent_vector
   ...
   class generic_range_type : public oneapi::tbb::blocked_range<Iterator> {
      using base_type = oneapi::tbb::blocked_range<Iterator>;
      ...
      template <typename U>
      generic_range_type(const generic_range_type<U>& r) : oneapi::tbb::blocked_range<Iterator>(r.begin(), r.end(), r.grainsize()) {}
      generic_range_type(generic_range_type& r, oneapi::tbb::split) : oneapi::tbb::blocked_range<Iterator>(r, oneapi::tbb::split()) {}
   ...
   using allocator_traits_type = oneapi::tbb::detail::allocator_traits<Allocator>;
   ...
   /**
    *  This method is a special way to move the list from other to this list.
    *  This function is required for the inline wrapper to make sure, that races are handled
    *  properly and the size is never touched accidentally.
    * @param other the other list
    */
   void initFrom(concurrent_vector&& other) {
      this->moveNoClear(std::move(other));
   }
   ...
   typename std::enable_if<oneapi::tbb::detail::is_input_iterator<ForwardIterator>::value, iterator>::type
   ...
   template <typename... Args>
   iterator emplace_back_noSizeAdjust(size_t oldSize, Args&&... args) {
      return internal_emplace_back_incrementedSize(oldSize, std::forward<Args>(args)...);
   }
   ...
   size_type size() const noexcept {
      auto theSize = this->my_size.load(std::memory_order_acquire);
      [[maybe_unused]] auto theCapacity = capacity();
   ...
   void reserve(size_type n) {
      ...
      if (n > max_size()) {
         oneapi::tbb::detail::throw_exception(oneapi::tbb::detail::d0::exception_id::reservation_length_error);
      }
   ...
   using segment_element_allocator_traits = oneapi::tbb::detail::allocator_traits<segment_element_allocator_type>;

   segment_table_type allocate_long_table(const typename base_type::atomic_segment* embedded_table, size_type start_index) {
      assert(start_index <= this->embedded_table_size && "Start index out of embedded table");
      ...
      for (segment_index_type i = 0; this->segment_base(i) < start_index; ++i) {
         oneapi::tbb::detail::d0::spin_wait_while_eq(embedded_table[i], segment_type(nullptr));
      }
   ...
   segment_type create_segment(segment_table_type table, segment_index_type seg_index, size_type index) {
      ...
      if (seg_index < first_block) {
         ...
         if (table[0].load(std::memory_order_acquire) != nullptr) {
            oneapi::tbb::detail::d0::spin_wait_while_eq(table[seg_index], segment_type(nullptr));
            return nullptr;
         }
         ...
         if (index == offset) {
            assert(table[seg_index].load(std::memory_order_relaxed) == nullptr && "Only this thread can enable this segment");
            segment_element_allocator_type segment_allocator(base_type::get_allocator());
   ...
   static constexpr bool is_first_element_in_segment(size_type index) {
      ...
      return oneapi::tbb::detail::d0::is_power_of_two_at_least(index, 2);
   }
   ...
   reference internal_subscript(size_type index) {
      assert(index < this->my_size.load(std::memory_order_relaxed) && "Invalid subscript index");
   ...
   reference internal_subscript_with_exceptions(size_type index) {
      if (index >= this->my_size.load(std::memory_order_acquire)) {
         oneapi::tbb::detail::throw_exception(oneapi::tbb::detail::d0::exception_id::out_of_range);
      }
   ...
   iterator internal_emplace_back(Args&&... args) {
      ...
      return internal_emplace_back_incrementedSize(old_size, std::forward<Args>(args)...);
   }

   template <typename... Args>
   iterator internal_emplace_back_incrementedSize(size_t old_size, Args&&... args) {
      ...
      auto value_guard = oneapi::tbb::detail::d0::make_raii_guard([&] {
   ...
   iterator internal_grow_to_at_least(size_type new_size, const Args&... args) {
      ...
      if (end_segment >= this->pointers_per_embedded_table &&
         ...
         oneapi::tbb::detail::d0::spin_wait_while_eq(this->my_segment_table, this->my_embedded_table);
      }
      ...
      for (segment_index_type seg_idx = 0; seg_idx <= end_segment; ++seg_idx) {
         if (this->get_table()[seg_idx].load(std::memory_order_relaxed) == nullptr) {
            oneapi::tbb::detail::d0::atomic_backoff backoff(true);
   ...
   void internal_resize(size_type n, const Args&... args) {
      ...
      [[maybe_unused]] size_type old_size = this->my_size.load(std::memory_order_acquire);
      assert(n > old_size && "only resize to larger sizes are supported");
      reserve(n);
...
template <typename It, typename Alloc = oneapi::tbb::cache_aligned_allocator<oneapi::tbb::detail::d0::iterator_value_t<It>>,
          typename = std::enable_if_t<oneapi::tbb::detail::d0::is_input_iterator_v<It>>,
          typename = std::enable_if_t<oneapi::tbb::detail::d0::is_allocator_v<Alloc>>>
concurrent_vector(It, It, Alloc = Alloc())
   -> concurrent_vector<oneapi::tbb::detail::d0::iterator_value_t<It>, Alloc>;
...
/*
template <typename T, typename Allocator>
oneapi::tbb::detail::synthesized_three_way_result<typename concurrent_vector<T, Allocator>::value_type>
operator<=>(const concurrent_vector<T, Allocator>& lhs,
            const concurrent_vector<T, Allocator>& rhs) {
   return std::lexicographical_compare_three_way(lhs.begin(), lhs.end(),
                                                 rhs.begin(), rhs.end(),
                                                 oneapi::tbb::detail::synthesized_three_way_comparator{});
}
*/
} // namespace factDB::infra::list::tbb
