...
class segment_table {
   ...
   using allocator_traits_type = oneapi::tbb::detail::allocator_traits<allocator_type>;
   using segment_table_allocator_type = typename allocator_traits_type::template rebind_alloc<atomic_segment>;

   protected:
   using segment_table_allocator_traits = oneapi::tbb::detail::allocator_traits<segment_table_allocator_type>;
   ...
   segment_table(const allocator_type& alloc = allocator_type())
      : my_size{}, my_segment_table_allocator(alloc), my_segment_table(nullptr), my_first_block{}, my_segment_table_allocation_failed{} {
   ...
   segment_table(const segment_table& other)
      : my_size{}, my_segment_table_allocator(segment_table_allocator_traits::select_on_container_copy_construction(other.my_segment_table_allocator)),
        my_segment_table(nullptr), my_first_block{}, my_segment_table_allocation_failed{} {
      my_segment_table.store(my_embedded_table, std::memory_order_relaxed);
      zero_table(my_embedded_table, pointers_per_embedded_table);
      oneapi::tbb::detail::try_call([&] {
   ...
   segment_table(segment_table&& other)
      : my_size{}, my_segment_table_allocator(std::move(other.my_segment_table_allocator)), my_segment_table(nullptr), my_first_block{}, my_segment_table_allocation_failed{} {
      my_segment_table.store(my_embedded_table, std::memory_order_relaxed);
      zero_table(my_embedded_table, pointers_per_embedded_table);
   ...
   void moveNoClear(segment_table&& other) {
      my_segment_table_allocator = std::move(other.my_segment_table_allocator);
      internal_move_noClear(std::move(other));
   }

   segment_table& operator=(const segment_table& other) {
      if (this != &other) {
         oneapi::tbb::detail::copy_assign_allocators(my_segment_table_allocator, other.my_segment_table_allocator);
         internal_transfer(other, copy_segment_body_type{*this});
      }
      return *this;
   }

   segment_table& operator=(segment_table&& other) noexcept(derived_type::is_noexcept_assignment) {
      ...
      if (this != &other) {
         oneapi::tbb::detail::d0::move_assign_allocators(my_segment_table_allocator, other.my_segment_table_allocator);
         internal_move_assign(std::move(other), oneapi::tbb::detail::disjunction<is_equal_type, pocma_type>());
      }
      return *this;
   }
   ...
   void enable_segment(segment_type& segment, segment_table_type table, segment_index_type seg_index, size_type index) {
      ...
      assert(segment != nullptr && "If create_segment returned nullptr, the element should be stored in the table");
   }
   ...
   void reserve(size_type n) {
      if (n > allocator_traits_type::max_size(my_segment_table_allocator)) {
         oneapi::tbb::detail::throw_exception(oneapi::tbb::detail::exception_id::reservation_length_error);
      }
   ...
   void extend_table_if_necessary(segment_table_type& table, size_type start_index, size_type end_index) {
      ...
      if (table == my_embedded_table && end_index > embedded_table_size) {
         if (start_index <= embedded_table_size) {
            ...
            oneapi::tbb::detail::d0::atomic_backoff backoff;
            do {
               if (my_segment_table_allocation_failed.load(std::memory_order_relaxed)) {
                  oneapi::tbb::detail::throw_exception(oneapi::tbb::detail::d0::exception_id::bad_alloc);
               }
               backoff.pause();
               table = my_segment_table.load(std::memory_order_acquire);
   ...
   static constexpr segment_index_type segment_index_of(size_type index) {
      return size_type(oneapi::tbb::detail::log2(uintptr_t(index | 1)));
   }
   ...
   void internal_move(segment_table&& other) {
      ...
      clear();
      my_size.store(other.my_size.load(std::memory_order_relaxed), std::memory_order_relaxed);
      internal_move_noClear(std::forward<segment_table>(other));
   }

   void internal_move_noClear(segment_table&& other) {
      const_cast<segment_type&>(segment_allocation_failure_tag) = reinterpret_cast<segment_type>(1);

      // NOTE: allocators should be equal
      assert(my_size.load() >= other.my_size.load());
   ...
   void internal_move_construct_with_allocator(segment_table&& other, const allocator_type&, /*is_always_equal = */ std::true_type) {
      internal_move(std::move(other));
   }
   ...
   static constexpr size_type embedded_table_size = pointers_per_embedded_table == 0 ? 0 : segment_size(pointers_per_embedded_table);
   ...
   value_type& internal_subscript(size_type index) {
      ...
      if constexpr (allow_out_of_range_access) {
