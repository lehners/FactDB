...
#include "factDB/infra/list/STLDequeCounter.hpp"
#include "tbb/scalable_allocator.h"
#include <deque>
// ---------------------------------------------------------------------------------------------------
namespace factDB::infra::list {
// ---------------------------------------------------------------------------------------------------
constexpr bool doCounts = false;
// ---------------------------------------------------------------------------------------------------
template <typename ElementTypeParam>
class NonOwningStlDeque;
// ---------------------------------------------------------------------------------------------------
template <typename ElementTypeParam>
class OwningStlDeque {
   public:
   friend class NonOwningStlDeque<ElementTypeParam>;
   using ElementType = ElementTypeParam;
   using NonOwningVariant = NonOwningStlDeque<ElementType>;
#if SCALABLE_ALLOC
   using ListType = std::deque<ElementType, ::tbb::scalable_allocator<ElementType>>;
#else
   using ListType = std::deque<ElementType>;
#endif
   ...
   public:
   OwningStlDeque() noexcept = default;
   ~OwningStlDeque() {
      if constexpr (doCounts)
         STLDequeStats::sizesOwningStlDeque.emplace_back(size());
   }

   OwningStlDeque(OwningStlDeque&& other) : list(std::move(other.list)) {}
   OwningStlDeque& operator=(OwningStlDeque&& other) {
   ...
   [[nodiscard]] bool operator==(const OwningStlDeque& other) const { return list == other.list; }
...
// ---------------------------------------------------------------------------------------------------
template <typename ElementTypeParam>
class OwningStlDequeLockable : public OwningStlDeque<ElementTypeParam> {
   public:
   std::mutex lock = {};

   OwningStlDequeLockable() = default;

   OwningStlDequeLockable(const OwningStlDequeLockable& other) = delete;
   OwningStlDequeLockable(OwningStlDequeLockable&& other)
      : OwningStlDeque<ElementTypeParam>(std::move(other)) {}

   OwningStlDequeLockable& operator=(const OwningStlDequeLockable& other) = delete;
   OwningStlDequeLockable& operator=(OwningStlDequeLockable&& other) {
      this->list = other.list;
      return *this;
   }

   ~OwningStlDequeLockable() {
      if constexpr (doCounts)
         STLDequeStats::sizesOwningStlDequeLocking.emplace_back(this->size());
   }
};
// ---------------------------------------------------------------------------------------------------
template <typename ElementTypeParam>
class NonOwningStlDeque {
   ...
   ~NonOwningStlDeque() {
      if constexpr (doCounts) {
         STLDequeStats::ptrsNonOwningStlDeque.emplace_back(list.size());
         STLDequeStats::sizesNonOwningStlDeque.emplace_back(sizeVal);
      }
   }

   private:
   using ElementType = ElementTypeParam;
   using OwningVariant = OwningStlDeque<ElementType>;
   ...
   void merge(NonOwningStlDeque& otherList) {
   ...
   bool operator==(const NonOwningStlDeque& other) const { return list == other.list; }
